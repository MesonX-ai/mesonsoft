#!/usr/bin/env bash
#
# deploy_script.sh — Mesonsoft deploy pipeline
#
#   1. Commit & push source to https://github.com/MesonX-ai/mesonsoft.git
...
#   ./deploy_script.sh --dry-run        # commit (no push) + build + preview what would upload
#   ./deploy_script.sh --skip-git       # build + upload only (no commit/push)
#   ./deploy_script.sh --skip-build     # git + upload only (reuse existing out/)
#   ./deploy_script.sh --skip-upload    # git + build only
#   ./deploy_script.sh --force          # ignore checksum manifest; upload all of out/
#
# Credentials come from the "Mesonsoft" entry in ftp-config.json (project dir
